...
Class Handler
{
    ...
    public function getSecurePath($file, $searchPaths = false)
    {
        if ($searchPaths) {
            ...
            return str_replace(array(APP, CLASSES, ROOT, OBULLO, PUBLIC_DIR, PRIVATE_DIR), $replace, $file);
        }
        if (is_string($file)) {
            ...
            if (strpos($file, PRIVATE_DIR) === 0) {
                $file = 'PRIVATE_DIR' . DS . substr($file, strlen(PRIVATE_DIR));
            }
        }
        return $file;
    }
     ...
     * 
     * @param mixed $string error string
     * 
     * @return mixed
     */
